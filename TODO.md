...
**In flight: PRJ-35** — the scaffold chart scan (`auto-tests/group-a/scan-scaffold-charts.bash`)
is written, proven in both directions and wired into both hosts; what is left is reading the
first real CI run.
...
| PRJ-35 | 🟡 | **The scaffold Helm chart is never scanned and the run stays green** | `checkov -d .` logs `[WARNI] Failed processing helm chart @@PROJECT@@ at dir: ./templates/service/helm` and skips the chart, while the same run prints `helm scan results: Passed checks: 84` for the repo's own `helm/` chart — which reads like coverage of both. Cause is the **rendered** output, not a broken template: `.Chart.Name` is the literal `@@PROJECT@@`, so `name: @@PROJECT@@` is emitted unquoted and `@` is a reserved YAML indicator. Fixed by `auto-tests/group-a/scan-scaffold-charts.bash` (substitute a throwaway copy → render → scan) plus a blocking job on both hosts. Verified locally and in the CI image: 11 resources, 168 checks passed; and red in five deliberate breakages (chart removed, unknown token, render error, violating resource, zero resources parsed). **Open: confirm the two CI jobs are green on a real run.** |
